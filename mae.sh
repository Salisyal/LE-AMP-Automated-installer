...
optMenu(){
    ...
    optApachePhalcon=$(echo -ne "MaeWS based on ${LRED}ApachePhalcon(tahap pengembangan)${RESET}")
    optExit=$(echo -ne "Exit from ${SPURPLE}MaeWS${RESET}")
    PS3="Select your Web Server: "

    select maeOpt in "${optNginx}" "${optApache}" "${optApachePhalcon}" "${optExit}"
    do
        case $maeOpt in
            "${optNginx}")
                cd tools
            ...
            "${optApachePhalcon}")
            cd tools
            chmod +x maews_apache_phalcon.sh
            ./maews_apache_phalcon.sh 
            break;;
            "${optExit}")
