...
${GREEN}/_/  /_/\_,_/\__/${SPURPLE}|__/|__/___/ ${RESET}v{1.2#${YELLOW}dev${RESET}} by @Salisyal thx to @sianturi1337
    ...
    if [[ ${OSTYPE} == "linux-gnu" ]]; then
        if [[ "$(cat /etc/os-release | grep PRETTY_NAME | cut -d '"' -f2 | awk '{print $1;}')" =~ ^(Ubuntu|Debian)$ ]]; then
            ...
            if command -v php &> /dev/null; then
                ...
                `sudo apt install php7.3 php7.3-curl php7.3-gettext php7.3-json php7.3-json php7.3-mbstring php7.3-pdo_* php7.3-fileinfo php7.3-mysql -y -qq &> /dev/null`
                echo -e "[${GREEN}DONE${RESET}]"
                echo -ne "[${LBLUE}${TIME}${RESET}] [${GREEN}INFO${RESET}] Installing PHP PHALCON ------->> "
                `git clone  https://github.com/phalcon/cphalcon /home cd "/home/cphalcon/build" git checkout origin/3.4.x sudo ./install -y -qq &> /dev/null`
                echo -e "[${GREEN}DONE${RESET}]"
